...
def sentiw(field):
    ...
    hope = os.path.join(module_dir, 'static/vader/vader_lexicon.txt')
    emojika = os.path.join(module_dir, 'static/vader/emoji_utf8_ENGLISH.txt')
    ...
    class SentimentIntensityAnalyzer(object):
        ...
        def sentiment_valence(self, valence, sentitext, item, i, sentiments):
            ...
            if item_lowercase in self.lexicon:
                ...
                if (i > 0 and words_and_emoticons[i - 1].lower() == "no") \
                        ...
                        or (
                        i > 2 and words_and_emoticons[i - 3].lower() == "no" and words_and_emoticons[i - 1].lower() in [
                    "or", "nor"]):
        ...
        def _negation_check(valence, words_and_emoticons, start_i, i):
            words_and_emoticons_lower = [str(w).lower() for w in words_and_emoticons]
            if start_i == 0:
                if negated([words_and_emoticons_lower[
                                i - (start_i + 1)]]):  # 1 word preceding lexicon word (w/o stopwords)
                    valence = valence * N_SCALAR
            if start_i == 1:
                if words_and_emoticons_lower[i - 2] == "never" and \
                    ...
                    valence = valence
                        ...
                        [words_and_emoticons_lower[i - (start_i + 1)]]):  # 2 words preceding the lexicon word position
                    valence = valence * N_SCALAR
            if start_i == 2:
                if words_and_emoticons_lower[i - 3] == "never" and \
                    ...
                    valence = valence
                        ...
                        [words_and_emoticons_lower[i - (start_i + 1)]]):  # 3 words preceding the lexicon word position
                    valence = valence * N_SCALAR
            return valence
    ...
    ss = analyzer.polarity_scores(field)
    print("{:-<65} {}".format(field, str(ss)))
    if (ss['compound'] >= 0.05):
        polarity = "Positive"
    ...
    arraylist = ['Review: ' + field, 'Negative: ' + str(ss['neg']), 'Neutral: ' + str(ss['neu']),
                 'Positive: ' + str(ss['pos']), \
